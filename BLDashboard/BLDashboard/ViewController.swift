...
//        Riffle.setFabric("ws://192.168.1.4:8000x/ws")
        ...
        bpm2 = Sensor(title: "BPM2", subtitle: "Voltage", dataValue: 0.0, threshold: 20.0, dataType: .VOLTS)
...
//    }
    func updateAccel(accel: [Double]){
        let listOfAccel: [Double] = [accel[0].roundToPlaces(2), accel[1].roundToPlaces(2), accel[2].roundToPlaces(2)] //3 decimal places
        vcmA.setValue(listOfAccel)
        RightMiddleInfo.update()
        updateGaugeUI()
        log.debug(vcmA.debug())
    ...
    func updateGyro(gyro: [Double]){
        let listOfGyro: [Double] = [gyro[0].roundToPlaces(2), gyro[1].roundToPlaces(2), gyro[2].roundToPlaces(2)] //3 decimal places
        vcmG.setValue(listOfGyro)
        updateGaugeUI()
        log.debug(vcmG.debug())
    }
